  ...
  const handleSelect = async (resource: any) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setReviewNote(result.resource.adminNotes || '');
      } else if (result.conflict) {
        setConflict(result);
      }
    } catch (e) {
      console.error('Open Review Error:', e);
      toast.error('Failed to lock resource for review');
    }
  };
  ...
  const handleAction = async (action: 'approve' | 'reject' | 'request') => {
    ...
    try {
      if (action === 'approve') {
        ...
        toast.success('Resource Certified & Distributed');
      } else if (action === 'reject') {
        if (!reviewNote) return toast.error('Rejection reason required');
        await rejectResource(selected._id, reviewNote, 'Policy Violation');
        toast.success('Resource Rejected');
      } else if (action === 'request') {
        if (!reviewNote) return toast.error('Change instructions required');
        ...
        <div className="lg:col-span-5 sticky top-8">
          <div className="glass-card overflow-hidden border-white/10 bg-slate-900/60 shadow-2xl min-h-[500px] flex flex-col">
            {selected ? (
              ...
              <div className="flex flex-col items-center justify-center h-full p-12 text-center space-y-6 text-amber-500">
                ...
                <button 
                  ...
                  className="px-8 py-3 bg-white/5 border border-white/10 rounded-full text-slate-300 text-[10px] font-black uppercase tracking-widest italic hover:bg-white/10 transition-all font-inter"
                >
                  Return to Queue
                </button>
              </div>
